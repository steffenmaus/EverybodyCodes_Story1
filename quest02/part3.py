...
class Node:
    ...
    def set_child_l(self, node):
        self.child_l = node

    def set_child_r(self, node):
        self.child_r = node
    ...
    def replace(self, rank, symbol, child_l, child_r):
        old_rank = self.rank
        old_symbol = self.symbol
        old_child_l = self.child_l
        old_child_r = self.child_r
        self.rank = rank
        self.symbol = symbol
        self.child_l = child_l
        self.child_r = child_r
        return (old_rank, old_symbol, old_child_l, old_child_r)
...
for l in lines:
    if l.startswith("ADD"):
        ...
        # SWAP
        l, r = nodes_per_id[l.split(" ")[1]]
        rank, symbol, child_l, child_r = l.replace(r.get_rank(), r.get_symbol(), r.get_child_l(), r.get_child_r())
        r.replace(rank, symbol, child_l, child_r)
...
def get_word_from_tree(root):
    ...
    word = "".join([n.get_symbol() for n in sorted(levels.items(), key=lambda t: (len(t[1]), -t[0]))[-1][1]])
    return word
